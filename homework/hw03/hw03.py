...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(position, increment, result):
    	if position==n:
    		return result
    	elif position%7==0 or num_sevens(position)!=0:
    		return helper(position+1, -1*increment, result-increment)
    	else:
    		return helper(position+1, increment, result+increment)
    return helper(1, 1, 1)
